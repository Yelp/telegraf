# Yelp Exec Input Plugin

The `yelp_exec` is a fork of the `exec` plugin which executes the `commands` on every interval and parses metrics from
...
The main difference between the `yelp_exec` and the `exec` plugin is that each `command` provided is prepended with `/bin/bash -c` before
execution. This allows for Bash variable interpretation as part of the provided command (e.g. `/bin/echo Hostname is $(hostname)`).
